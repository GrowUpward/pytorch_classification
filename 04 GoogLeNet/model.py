...
class InceptionAux(nn.Module):
    ...
    def forward(self,x):
        ...
        # N x 2048
        x=F.dropout(x,p=0.7,training=self.training)#训练模式下（model.train()）,training自动设置为true
        x=self.fc1(x)
        x=F.relu(x,inplace=True)
        # N x 1024
        x=F.dropout(x,p=0.7,training=self.training)#评估模式下（model.eval()）,training自动设置为false
        x=self.fc2(x)
        # N x num_classes
